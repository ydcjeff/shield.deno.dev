...
<svg viewBox="0 0 ${totalW} 20" width="${totalW}" height="20" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" role="img" aria-label="${ariaLabel}">
  ...
  <g fill="#fff" text-anchor="start" font-family="Verdana,sans-serif" font-size="11" text-rendering="geometricPrecision" dominant-baseline="middle">
    <text x="${denoLogo + (2 * gap)}" y="10" textLength="${labelTextLength}">${escapeHTML(label)}</text>
    <text x="${leftW + gap}" y="10" textLength="${msgTextLength}">${escapeHTML(msg)}</text>
  </g>
